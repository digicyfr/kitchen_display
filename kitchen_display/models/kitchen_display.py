...
class KitchenDisplay(models.Model):
    ...
    # Display Options - Card Fields
    show_customer_name = fields.Boolean(
        string='Show Customer Name',
        default=True,
        help="Display customer name on kitchen card"
    )
    show_table_number = fields.Boolean(
        string='Show Table Number',
        default=True,
        help="Display table number badge on kitchen card"
    )
    show_elapsed_time = fields.Boolean(
        string='Show Elapsed Time',
        default=True,
        help="Display elapsed time since order was placed"
    )
    show_order_time = fields.Boolean(
        string='Show Order Timestamp',
        default=False,
        help="Display exact order time at bottom of card"
    )
    show_items_summary = fields.Boolean(
        string='Show Items Summary',
        default=True,
        help="Display order items/products on the card"
    )
    show_special_instructions = fields.Boolean(
        string='Show Special Instructions Icon',
        default=True,
        help="Show warning icon when order has special instructions"
    )
    show_priority_badge = fields.Boolean(
        string='Show Priority Badge',
        default=True,
        help="Display priority indicator for urgent/high priority orders"
    )
    show_order_total = fields.Boolean(
        string='Show Order Total',
        default=False,
        help="Display order total amount on card"
    )
    show_preparation_time = fields.Boolean(
        string='Show Estimated Time',
        default=False,
        help="Display estimated preparation time"
    )
    ...
    # Visible States Configuration (for Kanban columns)
    # Default: Show only New, Preparing, Ready (3 columns)
    show_state_new = fields.Boolean(
        string='Show "New Order" Column',
        default=True,
        help="Display New Order column in kanban view (always visible even if empty)"
    )
    show_state_preparing = fields.Boolean(
        string='Show "Preparing" Column',
        default=True,
        help="Display Preparing column in kanban view (always visible even if empty)"
    )
    show_state_ready = fields.Boolean(
        string='Show "Ready for Pickup" Column',
        default=True,
        help="Display Ready for Pickup column in kanban view (always visible even if empty)"
    )
    show_state_served = fields.Boolean(
        string='Show "Served" Column',
        default=False,
        help="Display Served column in kanban view"
    )
    show_state_cancelled = fields.Boolean(
        string='Show "Cancelled" Column',
        default=False,
        help="Display Cancelled column in kanban view"
    )

    visible_states = fields.Char(
        string='Visible States',
        compute='_compute_visible_states',
        store=True,
        help="Comma-separated list of visible states for filtering"
    )

    @api.depends('show_state_new', 'show_state_preparing', 'show_state_ready',
                 'show_state_served', 'show_state_cancelled')
    def _compute_visible_states(self):
        """Compute comma-separated list of visible states"""
        for record in self:
            states = []
            if record.show_state_new:
                states.append('new')
            if record.show_state_preparing:
                states.append('preparing')
            if record.show_state_ready:
                states.append('ready')
            if record.show_state_served:
                states.append('served')
            if record.show_state_cancelled:
                states.append('cancelled')
            record.visible_states = ','.join(states) if states else 'new,preparing,ready'
    ...
    def action_open_kitchen_board(self):
        """Open kitchen board with filtered states based on display configuration"""
        self.ensure_one()

        # Get visible states (columns to show)
        visible_states = []
        if self.show_state_new:
            visible_states.append('new')
        if self.show_state_preparing:
            visible_states.append('preparing')
        if self.show_state_ready:
            visible_states.append('ready')
        if self.show_state_served:
            visible_states.append('served')
        if self.show_state_cancelled:
            visible_states.append('cancelled')

        # Default to showing new, preparing, ready if nothing selected
        if not visible_states:
            visible_states = ['new', 'preparing', 'ready']

        # Build domain to filter orders by visible states
        domain = [('state', 'in', visible_states)]

        # Add POS config filter if specified
        if self.pos_config_ids:
            domain.append(('pos_config_id', 'in', self.pos_config_ids.ids))

        # Add category filter if specified
        if self.category_ids:
            domain.append(('order_lines.product_id.categ_id', 'in', self.category_ids.ids))

        return {
            'name': f'Kitchen Board - {self.name}',
            'type': 'ir.actions.act_window',
            'res_model': 'kitchen.order',
            'view_mode': 'kanban',
            'view_id': self.env.ref('kitchen_display.kitchen_order_kanban_view').id,
            'target': 'current',
            'domain': domain,
            'context': {
                'kitchen_board': True,
                ...
                'visible_states': visible_states,  # List of states to show as columns
                'group_by': 'state',  # Ensure grouping by state
                'auto_refresh_interval': self.auto_refresh_interval,  # Auto-refresh every N seconds
                # Display configuration - what to show on cards
                ...
                'show_order_time': self.show_order_time,
                'show_items_summary': self.show_items_summary,
                'show_special_instructions': self.show_special_instructions,
                'show_priority_badge': self.show_priority_badge,
                'show_order_total': self.show_order_total,
                'show_preparation_time': self.show_preparation_time,
            }
        }